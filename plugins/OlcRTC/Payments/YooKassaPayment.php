    ...
    public function form(): array
    {
        return [
            ...
            'send_receipt' => [
                ...
                'default'     => false,
                'description' => 'Включите, если у вас подключена онлайн-касса через ЮKassa. Если выключено — поля «Код СНО» и «Ставка НДС» ниже можно ОСТАВИТЬ ПУСТЫМИ (не требуются).',
            ],
            'tax_system_code' => [
                'type'        => 'string',
                'label'       => 'Код СНО (только при включенной кассе 54-ФЗ)',
                'default'     => '',
                'placeholder' => 'например: 1 (оставьте пустым если нет кассы)',
                'description' => 'Доступные коды (пишите как цифру, можно с пробелами): 1 - ОСН, 2 - УСН доход, 3 - УСН доход-расход, 4 - ЕНВД, 5 - ЕСХН, 6 - ПСН. Оставьте ПУСТЫМ если галочка «Отправлять чек (54-ФЗ)» выключена.',
            ],
            'vat_code' => [
                'type'        => 'string',
                'label'       => 'Ставка НДС (только при включенной кассе 54-ФЗ)',
                'default'     => '',
                'placeholder' => 'например: 1 (оставьте пустым если нет кассы)',
                'description' => 'Доступные ставки: 1 - без НДС (обычно для ИП на УСН/ОСН без НДС в биллинге VPN), 2 - 0%, 3 - 10%, 4 - 20%, 5 - расчет 10/110, 6 - расчет 20/120. Оставьте ПУСТЫМ если галочка «Отправлять чек (54-ФЗ)» выключена.',
            ],
        ];
    }
