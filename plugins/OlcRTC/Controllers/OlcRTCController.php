    ...
    public function info(Request $request)
    {
        ...
        try {
            ...
            return $this->success([
                ...
                'banner'       =>
                    $uri
                        ? '💳 ВАШ КЛЮЧ OlcRTC — ВЫДАН! Нажмите «📋 СКОПИРОВАТЬ КЛЮЧ» ниже → вставьте в клиент OlcBox/owenclave → Connect. DNS Яндекс уже настроен, больше ничего менять не нужно.'
                        : '⏳ КЛЮЧ ГОТОВИТСЯ (1–3 мин после покупки/регистрации). Временные причины: olcrtc-менеджер создаёт инстанс, reconciler-крон ещё не пробежал, или подключение задержалось. Нажмите «🔄 Пересоздать инстанс» → подождите 1 минуту → обновите страницу.',
                'primary_cta'  =>
                    $uri
                        ? '📋 СКОПИРОВАТЬ КЛЮЧ OlcRTC (URI)'
                        : '🔄 ПЕРЕСОЗДАТЬ ИНСТАНС (получить ключ быстрее)',
                'support_hint' => 'Если ключа нет БОЛЕЕ 5 минут — напишите Тикет в разделе «🎫 Поддержка / Тикеты» ЛК, приложите скриншот ошибки, мы пересоздадим вручную за 5 минут.',
                'create_error' => $data['create_error'] ?? null,
            ]);
        } catch (\Throwable $e) {
