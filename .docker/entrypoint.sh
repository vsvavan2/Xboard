...
patch_admin_cjk_to_ru() {
    ...
    | xargs -0 sed -i \
        ...
        -e 's|Add to Chat|В поддержку|g' \
        -e 's|Трафик限制\(GB\)|Лимит трафика (GB)|g' \
        -e 's|0 表示不限制|0 = без ограничений|g' \
        -e 's|Кастомный ID узла\(\(Опционально\)\)|Кастомный ID узла (необязательно)|g' \
        -e 's|Кастомный ID узла\(\(Опционально\)\)|Кастомный ID узла (необязательно)|g' \
        -e 's|Группы доступаДобавить группу|Группы доступа · Добавить ↘|g' \
        -e 's|请选择分组|Выберите группу доступа (Для OlcRTC VPN — «Все пользователи VPN»)|g' \
        ...
        -e 's/帮助/Помощь/g' \
        -e 's/关于/О нас/g' \
        -e 's/添加服务器/Добавить сервер/g' \
        -e 's/服务器管理/Управление серверами/g' \
        -e 's/服务器/Сервер/g' \
        -e 's/添加节点/Добавить узел/g' \
        -e 's/节点管理/Управление узлами/g' \
        -e 's/节点/Узел/g' \
        -e 's/添加分组/Добавить группу/g' \
        -e 's/分组管理/Управление группами/g' \
        -e 's/添加线路/Добавить маршрут/g' \
        -e 's/线路管理/Управление маршрутами/g' \
        -e 's/添加套餐/Добавить план/g' \
        -e 's/套餐管理/Тарифные планы/g' \
        -e 's/分组/Группа/g' \
        -e 's/线路/Маршрут/g' \
        -e 's/套餐/План/g' \
        -e 's/搜索/Поиск/g' \
        -e 's/查询/Найти/g' \
        -e 's/搜索服务器/Поиск по имени сервера/g' \
        -e 's/搜索节点/Поиск узлов/g' \
        -e 's/搜索分组/Поиск групп/g' \
        -e 's/搜索线路/Поиск маршрутов/g' \
        -e 's/搜索套餐/Поиск планов/g' \
        -e 's/无数据/Нет данных/g' \
        -e 's/数据为空/Данные отсутствуют/g' \
        -e 's/选中/Выбрано/g' \
        -e 's/从0到0/из 0/g' \
        -e 's/运行状态/Статус/g' \
        -e 's/状态/Статус/g' \
        -e 's/负载/Нагрузка/g' \
        -e 's/最后响应/Последний отклик/g' \
        -e 's/操作/Действия/g' \
        -e 's/用户在线/Пользователей онлайн/g' \
        -e 's/倍率/Множитель/g' \
        -e 's/流量/Трафик/g' \
        -e 's/展开/Показ/g' \
        -e 's/显示/Показывать/g' \
        ...
        -e 's/续费/Продление/g' \
        -e 's/统计/Статистика/g' \
        -e 's/高负载/Высокая нагрузка/g' \
        -e 's/丢失/Потеряны/g' \
        -e 's/离线/Оффлайн/g' \
        -e 's/在线/Онлайн/g' \
        -e 's/总计/Всего/g' \
        -e 's/共计/Всего/g' \
        -e 's/服务器在线/Серверы онлайн/g' \
        -e 's/总计服务器/Всего серверов/g' \
        -e 's/离线丢失/Оффлайн_Потеряны/g' \
        -e 's/按服务器名称备注SID搜索/Поиск по имени сервера, заметкам или SID.../g' \
        -e 's/服务器名称/Имя сервера/g' \
        -e 's/服务器备注/Заметка сервера/g' \
        -e 's/节点数量/Количество узлов/g' \
        -e 's/节点类型/Тип узла/g' \
        -e 's/部署方式/Развёртывание/g' \
        -e 's/地址/Адрес/g' \
        -e 's/节点ID/ID узла/g' \
        -e 's/节点备注/Заметка узла/g' \
        -e 's/节点名称/Имя узла/g' \
        -e 's/接入组/Группы доступа/g' \
        -e 's/访问组/Группы доступа/g' \
        -e 's/分组名称/Название группы/g' \
        -e 's/分组ID/ID группы/g' \
        -e 's/线路名称/Примечание маршрута/g' \
        -e 's/线路备注/Примечание маршрута/g' \
        -e 's/线路ID/ID группы маршрута/g' \
        -e 's/动作值/Значение действия/g' \
        -e 's/动作/Действие/g' \
        -e 's/套餐名称/Название/g' \
        -e 's/套餐ID/ID/g' \
        -e 's/销售价格/Цена/g' \
        -e 's/价格/Цена/g' \
        -e 's/立即体验/Попробовать/g' \
        -e 's/请输入/Введите/g' \
        -e 's/请选择/Выберите/g' \
        -e 's/保存提交/Сохранить/g' \
        -e 's/保存修改/Сохранить/g' \
        -e 's/编辑配置/Конфигурация/g' \
        -e 's/修改插件设置/Изменение настроек плагина/g' \
        -e 's/保存并返回/Сохранить и вернуться/g' \
        -e 's/返回管理/Назад к списку/g' \
        -e 's/全部/Все/g' \
        -e 's/启用/Вкл/g' \
        -e 's/禁用/Выкл/g' \
        -e 's/删除确认/Подтверждение удаления/g' \
        -e 's/确定要删除/Вы действительно хотите удалить/g' \
        -e 's/确认/Ок/g' \
        -e 's/提示信息/Подсказка/g' \
        -e 's/操作成功/Операция успешна/g' \
        -e 's/操作失败/Ошибка операции/g' \
        -e 's/上传成功/Загружено успешно/g' \
        -e 's/上传失败/Ошибка загрузки/g' \
        -e 's/请选择要操作的数据/Выберите элементы для действия/g' \
        -e 's/服务器管理运营视图/Управление серверами. Просматривайте состояние сервера, нагрузку и размещённые узлы, а также запускайте операции с узлами из операционного представления./g' \
        -e 's/服务器管理运营界面/Управление серверами/g' \
        -e 's/此页面可快速评估服务器状态节点数量资源负载情况/Страница помогает быстро оценить состояние серверов, число размещённых узлов и нагрузку ресурсов./g' \
        -e 's/节点管理运营视图/Управление всеми узлами, включая добавление, удаление, редактирование и другие операции./g' \
        -e 's/节点管理运营界面/Управление узлами/g' \
        -e 's/分组管理运营视图/Управление всеми группами прав, включая добавление, удаление и редактирование./g' \
        -e 's/分组管理运营界面/Группы прав/g' \
        -e 's/线路管理运营视图/Управление группами маршрутов, включая добавление, удаление и редактирование./g' \
        -e 's/线路管理运营界面/Управление маршрутами/g' \
        -e 's/套餐管理运营视图/Здесь вы можете настраивать тарифные планы, включая операции добавления, удаления и редактирования./g' \
